...
class TestNegativeEigenvalueHandling:
    ...
    def test_nan_handling(self):
        ...
        assert torch.isnan(torch.tensor(fd)).item(), "Expected NaN for rank-deficient features with 'nan' handling"
    ...
    def test_clamp_handling(self):
        ...
        assert fd == 0.0, f"Expected 0.0 for rank-deficient features with 'clamp' handling, got {fd}"
    ...
    def test_warn_handling(self):
        ...
        calc = FrechetDistanceCalculator(
            dataset="mmnist_stochastic",
            device="cpu",
            negative_eigenvalue_handling="warn"
        )
        
        with pytest.warns(RuntimeWarning, match="Negative eigenvalues detected"):
            fd = calc._calculate_frechet_distance_from_features(features1, features2)
        
        assert torch.isnan(torch.tensor(fd)).item(), "Expected NaN for rank-deficient features with 'warn' handling"
    ...
    def test_raise_handling(self):
        ...
        calc = FrechetDistanceCalculator(
            dataset="mmnist_stochastic",
            device="cpu",
            negative_eigenvalue_handling="raise"
        )
    ...
    def test_normal_features_all_options(self):
        ...
        assert not torch.isnan(torch.tensor(fd_clamp)).item(), "Unexpected NaN with 'clamp' handling for normal features"
